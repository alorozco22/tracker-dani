...
function agregarCardsActividad(actividades){
	...
	for (let i = 0; i<actividadesVisibles.length; i++){
		...
		console.log('actividades[i].recursos[0]');
		console.log(actividades[i].recursos[0]);
		includeListItem(actividades[i].recursos[0], actividades[i].links[0], '.rec1', newCard);
		includeListItem(actividades[i].recursos[1], actividades[i].links[1], '.rec2', newCard);
		includeListItem(actividades[i].recursos[2], actividades[i].links[2], '.rec3', newCard);
		...
		includeFreeItem(actividades[i].html, newCard)
...
function includeListItem(recurso, link, className, newCard){
	console.log(className);
	console.log(recurso);
	if(!(recurso == 'nan')){
		console.log("ENTERED TO includeListItem");
		console.log(recurso);
		newCard.querySelector(className).innerHTML = '<a target="_blank" href="'.concat(link.concat('">'.concat(recurso.concat('</a>'))));
	} else {
		newCard.querySelector(className).style.display = 'none';
	}	
}

function includeFreeItem(htm, newCard){
	
	if (!(htm == 'nan')){
		newCard.querySelector(".componente-libre").innerHTML = htm;
	} else {
		newCard.querySelector(".componente-libre").style.display = 'none';
	}
}